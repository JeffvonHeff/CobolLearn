...
def generate_transaction_timestamp():
    ...
    return transaction_timestamp.strftime(
        "%Y-%m-%d-%H.%M.%S.%f"
    )  # Timestamp i ønsket format
...
bank_registrations = [
    bank[0] for bank in bank_data
]  # Liste med alle registreringsnumre fra bankfilen
...
with open(TRANSACTION_FILE, "w") as file:
    for i in range(1, NUM_CUSTOMERS + 1):  # Loop over kunder
        ...
        reg_nummer = random.choice(
            bank_registrations
        )  # Vælg et tilfældigt registreringsnummer fra bankfilen
        navn = (
            f"{random.choice(FIRST_NAMES)} {random.choice(LAST_NAMES)}"  # Fiktivt navn
        )
        ...
        for _ in range(num_transactions):  # Loop over kundens transaktioner
            ...
            )  # Timestamp i formatet YYYY-MM-DD-HH.MM.SS.MMMMMM
...
print(
    f"Dataset med {NUM_CUSTOMERS} kunder og op til {MAX_TRANSACTIONS_PER_CUSTOMER * NUM_CUSTOMERS} transaktioner er genereret i filen '{TRANSACTION_FILE}'"
)
