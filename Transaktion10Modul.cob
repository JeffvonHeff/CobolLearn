       ...
       PROCEDURE DIVISION USING BY REFERENCE P-TRANSAKTION-REKORD
           ...
           PERFORM VARYING WS-IX FROM 1 BY 1 UNTIL WS-IX >= P-BANK-COUNT
               IF REG-NR OF P-BANK-ARRAY(WS-IX) =
                  REG-NR OF P-TRANSAKTION-REKORD
                   ...
                   MOVE SPACES TO WS-BELOB-DKK-OUT
                   MOVE WS-BELOB-DKK TO WS-BELOB-DKK-DISP
                   IF WS-BELOB-DKK < 0
                       MOVE FUNCTION ABS(WS-BELOB-DKK)
                           TO WS-BELOB-DKK-DISP
                       MOVE "-" TO WS-BELOB-DKK-OUT(1:1)
                       MOVE FUNCTION TRIM(WS-BELOB-DKK-DISP)
                   ...
                   STRING "Dato: " TIDSPUNKT OF P-TRANSAKTION-REKORD
                          " Transaktionstype: "
                          TRANSAKTIONSTYPE OF P-TRANSAKTION-REKORD
                          " Beløb (DKK): " WS-BELOB-DKK-OUT
                          " Butik: " BUTIK OF P-TRANSAKTION-REKORD
                       DELIMITED BY SIZE
                       INTO P-OUTPUT-TEXT
                   END-STRING
                   MOVE "Y" TO P-FOUND
